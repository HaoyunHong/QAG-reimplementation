...
class DataLoader(object):
    ...
    def __init__(self, path):
        ...
        for i, pair in enumerate(train_data):
            _length1, _length2, _keyword1, _keyword2, _jaccard, _inverse_pairs = self.preprocess(title2vec1[i], title2vec2[i],max_seq_len)
        ...
        for i in range(len(new_seq2)):
            for j in range(i, len(new_seq2)):
                if new_seq2[j] < i + 1:
                    result -= 1
        return result, \
               [0] * (max_seq_len - len(new_seq1)) + new_seq1 if len(new_seq1) <= max_seq_len else new_seq1[:max_seq_len], \
               [0] * (max_seq_len - len(new_seq2)) + new_seq2 if len(new_seq2) <= max_seq_len else new_seq2[:max_seq_len]
